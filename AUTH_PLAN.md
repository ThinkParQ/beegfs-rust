...
peer fails.
In addition, the nodes should periodically check the current generation based on timer / during
internodesyncer cycle to make sure to get informed about revocations quickly.
...
-> Claude recommends using an extra file, because an x509 server certificate uses rsa or ecdsa, and the
