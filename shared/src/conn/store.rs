...
pub struct Store {
    ...
    streams: Mutex<HashMap<NodeUID, (Arc<AsyncQueue<StoredStream>>, Arc<Semaphore>)>>,
    bufs: Mutex<VecDeque<MsgBuf>>,
    addrs: RwLock<HashMap<NodeUID, Arc<[SocketAddr]>>>,
...
impl Store {
    ...
    pub fn try_pop_stream(&self, node_uid: NodeUID) -> Option<StoredStream> {
    ...
    pub fn try_acquire_permit(&self, node_uid: NodeUID) -> Option<StoredStreamPermit> {
        let mut streams = self.streams.lock().unwrap();
    ...
    pub async fn pop_stream(&self, node_uid: NodeUID) -> Result<StoredStream> {
        let queue = {
            let mut streams = self.streams.lock().unwrap();
    ...
    pub fn get_node_addrs(&self, node_uid: NodeUID) -> Option<Arc<[SocketAddr]>> {
