...
impl PartialEq for BuddyGroupId {
    fn eq(&self, other: &Self) -> bool {
        self.id == other.id
    }
}

impl PartialOrd for BuddyGroupId {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        self.id.partial_cmp(&other.id)
    }
}

impl Hash for BuddyGroupId {
...
impl Display for BuddyGroupId {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(ref info) = self.info {
            ...
            write!(f, "buddy_group_id:{}", self.id)
        }
    }
}
...
impl TryFrom<u32> for BuddyGroupId {
...
impl TryFrom<i32> for BuddyGroupId {
    type Error = anyhow::Error;
...
#[cfg(feature = "sqlite")]
impl FromSql for BuddyGroupId {
    fn column_result(value: rusqlite::types::ValueRef) -> rusqlite::types::FromSqlResult<Self> {
        let id = value.as_i64()?;
        let id = id
            .try_into()
            .map_err(|_| rusqlite::types::FromSqlError::OutOfRange(id))?;
        Ok(BuddyGroupId { id, info: None })
    }
}

#[cfg(feature = "sqlite")]
impl ToSql for BuddyGroupId {
...
#[cfg(feature = "sqlite")]
impl From<BuddyGroupId> for Value {
    fn from(value: BuddyGroupId) -> Self {
