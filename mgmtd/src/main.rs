...
fn inner_main() -> Result<()> {
    ...
    if user_config.gen_key {
        return gen_key(&user_config.key_file);
    }
...
If you want to initialize a new system, refer to --help or doc.beegfs.io.",
    ...
    let (auth_secret, static_keypair) = if !user_config.auth_disable {
        ...
        // The long-term identity used by the BeeMsg key exchange. Peers accept a connection only if
        // the matching public key is registered in the management database.
        let keypair = StaticKeypair::from_key_file(&user_config.key_file)?;
...
/// Generates this nodes long-term X25519 keypair and writes the private half to `key_file`.
///
/// Refuses to overwrite an existing file: replacing a nodes key silently invalidates its registered
/// public key, and every peer would reject it until the new one is registered. The file is created
/// with owner-only permissions - it is a private key.
fn gen_key(key_file: &Path) -> Result<()> {
    ...
    if key_file.exists() {
        anyhow::bail!(
            "A key file already exists at {key_file:?}. Refusing to overwrite it - replacing this \
nodes key requires registering the new public key with management first. Delete the file \
explicitly if that is what you want."
    ...
    if let Some(dir) = key_file.parent() {
        fs::create_dir_all(dir)
            .with_context(|| format!("Could not create directory for {key_file:?}"))?;
    }
    ...
    let mut file = fs::OpenOptions::new()
        ...
        .mode(0o600)
        .open(key_file)
        .with_context(|| format!("Could not create key file {key_file:?}"))?;

    file.write_all(keypair.secret_bytes().as_slice())
        .with_context(|| format!("Could not write key file {key_file:?}"))?;

    println!("Private key written to {key_file:?}");
    println!("Public key: {}", keypair.public());
    println!();
    println!("Register it with management, e.g.:");
    println!("  INSERT INTO identities (name) VALUES ('<node-alias>');");
    println!(
        "  INSERT INTO keys (key, identity_id) VALUES (x'{}', last_insert_rowid());",
        keypair.public()
    );
...
fn init_db(db_file: &Path, v7_path: Option<&Path>, fs_uuid: Option<Uuid>) -> Result<()> {
    ...
    (|| -> Result<_> {
        ...
        conn.backup(rusqlite::MAIN_DB, db_file, None)
            .inspect_err(|_| {
                if let Err(err) = std::fs::remove_file(db_file) {
                    println!("Cleaning up {db_file:?} failed: {err}");
                }
            })?;
