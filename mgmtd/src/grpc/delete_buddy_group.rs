...
pub(crate) async fn delete_buddy_group(
    ...
    let p_res: RemoveBuddyGroupResp = app.beemsg_request(p_node_uid, &remove_bee_msg).await?;
    let s_res: RemoveBuddyGroupResp = app.beemsg_request(s_node_uid, &remove_bee_msg).await?;
