...
pub(crate) fn start_tasks(app: App, run_state: RunStateHandle) {
    ...
    if app.info.user_config.quota_enable {
        if let Err(err) = app.license.verify_feature(LicensedFeature::Quota) {
            log::error!(
...
async fn delete_stale_clients(app: App, mut run_state: RunStateHandle) {
...
async fn update_quota(app: App, mut run_state: RunStateHandle) {
    loop {
...
async fn switchover(app: App, mut run_state: RunStateHandle) {
